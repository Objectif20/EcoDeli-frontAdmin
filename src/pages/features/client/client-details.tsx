  ...
  return (
    <div className="container mx-auto p-4 max-w-6xl">
      <div className="mb-6">
        <Button
          variant="ghost"
          onClick={() => navigate('/office/profile/clients')}
          className="mb-4"
        >
          <ArrowLeft className="mr-2 h-4 w-4" />
          Retour à la liste
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center gap-4">
            <Avatar className="h-16 w-16 border-2 border-primary/10">
              <AvatarImage src={clientDetails.info.profile_picture || ''} alt={`${clientDetails.info.first_name} ${clientDetails.info.last_name}`} />
              <AvatarFallback>{getInitials(`${clientDetails.info.first_name} ${clientDetails.info.last_name}`)}</AvatarFallback>
            </Avatar>
            <div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <p>{clientDetails.info.email}</p>
              </div>
              <div className="flex items-center gap-2">
                <Phone className="h-4 w-4 text-muted-foreground" />
                <p>{clientDetails.info.phone}</p>
              </div>
            </div>
            <div>
              <h3 className="font-medium text-sm text-muted-foreground mb-1">Abonnement</h3>
              <p>{clientDetails.info.nomAbonnement}</p>
            </div>
          </div>
        </CardContent>
      </Card>
