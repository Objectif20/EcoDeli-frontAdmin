...
export default function ProviderContractPage() {
  ...
  const [_, setPrestataires] = useState<Prestataire[]>([])
  ...
  const fetchPrestataires = useCallback(
    async (page = 1, search = "") => {
      ...
      try {
        ...
        setPrestataires(mockData)
        setFilteredPrestataires(paginatedData)
        setTotalPages(total)
      } catch (error) {
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-full overflow-hidden bg-muted">
                        <img
                          src={`https://ui-avatars.com/api/?name=${prestataire.prenom}+${prestataire.nom}&background=random`}
