import { Ticket, TicketService } from "@/api/ticket.api";
...
export default function TicketUnique() {
  ...
  const canEdit =
    !!admin &&
    (admin.admin_id === ticket?.admin_id_get ||
      admin.admin_id === ticket?.admin_id_attribute ||
      admin.roles.includes("TICKET"));
  ...
  const form = useForm<TicketFormValues>({
    ...
    defaultValues: {
      ...
      priority: (ticket?.priority as "High" | "Medium" | "Low") || "Medium",
      state: (ticket?.state as "Pending" | "Progress" | "Done") || "Pending",
      admin_id_get: ticket?.admin_id_get || "",
    ...
    TicketService.getTicketById(id)
      .then((ticket) => {
        ...
        form.reset({
          ...
          state: ticket.state as "Pending" | "Progress" | "Done",
          admin_id_get: ticket.admin_id_get,
          admin_id_attribute: admin?.admin_id,
        });
      })
  ...
  useEffect(() => {
    if (ticket) {
      dispatch(
        setBreadcrumb({
          segments: ["Accueil", "Ticket", ticket.title || ""],
  ...
  const handleSubmit = async (values: TicketFormValues) => {
    ...
    try {
      await TicketService.updateTicket(ticket?.ticket_id || "", updatedValues);
    } catch (error) {
  ...
  if (!ticket) {
    return <div className="text-center py-10">Chargement du ticket...</div>;
  }
