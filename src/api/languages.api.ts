...
export async function getLanguages(page : number, limit : number) : Promise<{data: Language[], meta : {total: number, page: number, lastPage: number}}> {
  try {
...
export async function addLanguage(data : CreateLanguage, languages : File) : Promise<Language | {message : string}> {
  try {
...
export async function updateLanguage(id : string, data ?: UpdateLanguage, languages ?: File) : Promise<Language | {message : string}> {
  try {
    const formData = new FormData();
    if (data) {
